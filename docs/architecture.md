...
Word-boundary anchoring cannot rescue an entry that is genuinely a substring of a non-compliant license *name*, so two allowlist entries were removed in the same change: bare `BSL` (which matched "Business Source License (BSL 1.1)", not open source) is replaced by the precise `BSL-1.0` for Boost, and `Copyright` (which matched virtually any proprietary notice) is dropped entirely — a bare copyright notice names no license, and `config/exceptions.json` is the intended mechanism for a package whose registry metadata is odd.
