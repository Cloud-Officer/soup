...
- `apply_cached_metadata(key, package)`: Looks the package up by its qualified key (falling back to the bare name only for a legacy entry with no recorded language), copies the four verification fields from the cache onto the package, and for a package whose registry lookup failed this run first calls `restore_unresolved_metadata`
