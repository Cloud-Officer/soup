...
- `migrate_legacy_cache_keys(cache)`: Moves entries from caches written before packages were keyed by language (bare package-name keys) to their `<language>:<package>` key, using the `language` each entry recorded, so an entry is never applied to a same-named package from another ecosystem; an existing qualified key wins over a migrated one, and an entry with no recorded language keeps its bare key
- `apply_cached_metadata(key, package)`: Looks the package up by its qualified key (falling back to the bare name only for a legacy entry with no recorded language) and copies `risk_level`, `requirements`, and `verification_reasoning` from the cache onto the package, each coerced to a string so an entry lacking one of those keys leaves the field missing (prompted, defaulted by `--auto_reply`, or rejected by `--no_prompt`) rather than crashing. `last_verified_at` is copied only when the cached entry records the same version, so a version change is re-stamped with the current date instead of asserting the older verification. For a package whose registry lookup failed this run it first calls `restore_unresolved_metadata`
...
- `check_packages`: Validates licenses, then for each package applies cached metadata, applies the transitive-dependency defaults (`apply_dependency_defaults` writes the lowest risk level and `DEPENDENCY_TEXT`), prompts for anything still missing (a prompt answer is stripped, so a blank or whitespace-only answer counts as missing), stamps `last_verified_at` when it is still blank, and appends the markdown row
- `save_files`: The success path. Writes cache and markdown documentation files, replacing both wholesale — which is what lets a removed dependency drop out of the register. Only reached when every package completed `check_packages`. A run that detected no packages at all (every parser skipped, invoked from the wrong directory, lockfiles missing) writes neither file and warns on stderr instead, so an empty detection set can never wipe the register
...
- `collect_packages(results, packages)`: Compacts the fetched results and keys them by `Package#key` (`<language>:<package>`) into the packages hash, so same-named packages from different ecosystems do not overwrite each other
...
- `parse(file, packages)`: Parses lock file via `Bundler::LockfileParser` and fetches package details from RubyGems, fetching metadata for all specs in parallel via the inherited `parallel_each` helper (`BaseParser`). Direct dependencies are the lock file's `DEPENDENCIES` section, which lists exactly the gems declared in the `Gemfile`. `Bundler::LockfileParser#specs` never contains bundler itself, so when the `Gemfile` declares `bundler` the lock file's `BUNDLED WITH` version is fetched through `fetch_package` like any other direct gem (`declared_bundler`); without a `BUNDLED WITH` section it is not recorded. Any other declared gem with no locked spec (e.g. a gem restricted to another platform) is skipped without a lookup
- `fetch_package(...)`: Builds the gem from `rubygems_response`, or records it via `unresolved_package` (its single unresolved exit) when that returns nil, rather than aborting the scan
- `rubygems_response(spec)`: Walks `rubygems_candidates` in order and returns the first 200. It queries the versioned RubyGems endpoint first; when that version is not published (e.g. a platform-specific or yanked release) the gem's latest version is resolved and that version's endpoint re-queried. A swallowed network fault or empty body at any step short-circuits the chain — the registry is unreachable after every retry, so the remaining fallbacks would fail identically — and every non-200 dead end warns via `http_error_message`
- `rubygems_candidates(spec)` / `rubygems_version_url(name, version)`: A lazy enumerator of `[url, label, final]` candidates — the pinned version URL, then the latest version URL, whose `latest.json` lookup (through `successful_registry_response`) only runs once the pinned version has failed and ends the chain when it fails
...
- `parse(file, packages)`: Parses lock file and fetches package details from NPM registry in parallel via the inherited `parallel_each` helper (`BaseParser`). Requires `lockfileVersion` 2 or later (a lock file with no `packages` key raises `UnsupportedFormatError`); `dev` entries are excluded, and so are first-party entries that are never installed from the registry — workspace directories whose key carries no `node_modules/` segment (e.g. `packages/app`, and the root `""` key), `"link": true` workspace symlinks, and packages whose `resolved` is a `file:` path — so they are neither looked up nor recorded (`third_party_entry?`). Direct dependencies are the exact `dependencies`/`devDependencies` keys of the sibling `package.json`
...
- Reuses the npm helpers in `BaseParser` — `npm_registry_url`, `successful_registry_response`, `lookup_npm_registry_version`, and `build_npm_registry_package` — rather than holding its own registry URL. Every importmap pin is recorded as a direct dependency (`dependency: false`)
