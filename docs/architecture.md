...
- `apply_cached_metadata(name, package)`: Copies `risk_level`, `requirements`, and `verification_reasoning` from the cache onto the package. `last_verified_at` is copied only when the cached entry records the same version, so a version change is re-stamped with the current date instead of asserting the older verification. For a package whose registry lookup failed this run it first calls `restore_unresolved_metadata`
